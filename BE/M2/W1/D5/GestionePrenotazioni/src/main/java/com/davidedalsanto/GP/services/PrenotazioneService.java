...
public class PrenotazioneService {
	...
	public Prenotazione prenota(LocalDate data, long ut_id, long pos_id) throws UtenteNotFoundException, Exception {
		
		Utente ute = uteSer.findUtenteById(ut_id);
		Optional<Postazione> pos = posSer.findByIdPostazione(pos_id);
		
		if(ute != null && pos.isPresent()) {
			
			Postazione post = pos.get();
			
			//controllo che utente non abbia gia una prenotazione per quel giorno
			List<Prenotazione> sameDayPrenot = prerep.findByUtenteAndData(ute, data);
			
			//controllo che la postazione non sia gia prenotata per quel giorno
			List<Prenotazione> samePostzPrenot = prerep.findByPostazioneAndData(post, data);
			
			if(sameDayPrenot.size() != 0 || samePostzPrenot.size() != 0) {
				throw new Exception("Impossibile prenotare, hai gia' una prenotazione in data " + data + " o la postazione e' gia prenotata");
			}else {
		...
		throw new Exception("Impossibile prenotare, qualcosa e' andato storto.");
	}
